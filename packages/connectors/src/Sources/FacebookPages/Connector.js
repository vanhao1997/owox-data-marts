...
var FacebookPagesConnector = class FacebookPagesConnector extends AbstractConnector {
  ...
  async startImportProcess() {
    ...
    for (const nodeName in dataSources) {
      ...
      const fields = dataSources[nodeName] || [];
      ...
      if (!isTimeSeriesNode) {
        for (const pageId of configuredPageIds) {
          this.config.logMessage(`Fetching Facebook Page ${pageId} profile metadata`);
      ...
      for (let day = 0; day < Math.max(0, effectiveDays); day += 1) {
        ...
        for (const pageId of configuredPageIds) {
          this.config.logMessage(
            `Start importing Facebook Page ${pageId} for ${DateUtils.formatDate(currentDate)} (Page Insights freshness is usually T+1 / approximately 24 hours)`
          );
          ...
          this.config.logMessage(
            data.length
              ? `${data.length} records were fetched for Facebook Page ${pageId}`
